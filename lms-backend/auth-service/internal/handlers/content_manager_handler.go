...
func NewContentManagerHandler(db *gorm.DB) *ContentManagerHandler {
	repo := repositories.NewContentManagerRepository(db) // 1️Create Repo
	service := services.NewContentManagerService(repo)   // 2️ Pass Repo to Service
	return &ContentManagerHandler{service: service}      // 3️ Inject Correctly
}
